...
```{r}
calculations <- table(svmPred, test_data$credit_risk)
calculations
```
...
```{r}
#--- it is valuable t have a test dataset for a few reasons.  First it will help in the selection of a model to use to analyze a data set. Second it will help remove any bias in your analysis because it will be viewed as new data to the model. 
#--- Potential challanges might include data privacy and data security
```
...
```{r}
#--- there may be bias my using the applicant's age.  You could try rerunning the model without considering age and then comparing the results.
```

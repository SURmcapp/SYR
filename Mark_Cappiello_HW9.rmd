...
```{r}
#--- Based on size and color, the top words in the word cloud are 1/2, I, snow, syracuse, name, plow.
```
...
#--- Create a named list of word counts by frequency
...
```{r}
#--- The sorted list of word counts shows the most frequently occurring words in the corpus.
#--- the most frequently used words are listed out for you and to me it is earier to see these words in a list rather than in a word cloud
```
...
```{r}
#--- URLs for the positive and negative words lists
...
positive_words <- scan(positive_words_url, what = "character", comment.char = ";")
length(positive_words)

#--- Read in the list of negative words
negative_words <- scan(negative_words_url, what = "character", comment.char = ";")
...
dfm_positive <- dfm_match(dfm_data, features = positive_words)

#--- Calculate word frequencies of the matched positive words
positive_word_frequencies <- textstat_frequency(dfm_positive)

#--- Show the positive word frequencies
...
```{r}
sum_positive_words <- sum(positive_word_frequencies$frequency)
sum_positive_words
```
...
dfm_negative <- dfm_match(dfm_data, features = negative_words)

#--- Calculate word frequencies of the matched negative words
negative_word_frequencies <- textstat_frequency(dfm_negative)

#--- Show the negative word frequencies
negative_word_frequencies

#--- Show the sum of all negative words
sum_negative_words <- sum(negative_word_frequencies$frequency)
sum_negative_words
```
...
```{r}
#--- There are more than three times the number of positive words than negative words. Some negative words may not be used in a negative way, it depends on the context in which they are used.  The same holds true for positive words. 
```
